...
#' @examples
#' \dontrun{
#' utl_mg_load_excel_deprecated(
#'   "L1/rls_example.xlsx",
#'   "reef-life-survey-data-marinegeo-v1",
#'   "DATA"
#' )
#' }
  ...
  tryCatch(
    {
      if (output_table == "reef-life-survey-data-marinegeo-v1") {
        ...
        missing_columns <- dplyr::setdiff(
          c("Total", "Method", "Site No.", "P-Qs"),
          colnames(df_raw)
        )
        ...
        if (dplyr::last(runs$values) & length(runs$lengths) == 1) {
          ...
          print(paste0(
            "dropped ",
            as.character(dplyr::last(runs$lengths)),
            " rows containing 0 total count from the end of the datasheet"
          ))
        } else {
          ...
          dplyr::mutate(
            Time = dplyr::case_when(
              stringr::str_starts(Time, "1899-12-31") ~ trimws(
                gsub("T", "", gsub("Z", "", gsub("1899-12-31", "", Time)))
              ),
              T ~ Time
            )
          ) |>
          marinegeo.utils::utl_join_taxonomy_by_scientific_name_deprecated(
        ...
        message(paste(
          "Target table is not defined in utl_mg_load_excel_deprecated(): ",
          output_table
        ))
        return(NULL)
      }
...
#' @examples
#' \dontrun{
#' df <- utl_mg_load_excel_deprecated(
#'   "inst/extdata/test_rls_data_EPA_one_header_row.xlsx",
#'   "reef-life-survey-data-marinegeo-v1",
#'   "DATA"
#' ) |>
#'   dplyr::filter(Method == "1" | Method == "2")
#'
#' utl_join_taxonomy_by_scientific_name_deprecated(
#'   df,
#'   identification_column_name = "Species"
#' )
#'
...
#' )
#' }
utl_join_taxonomy_by_scientific_name_deprecated <- function(
  df,
  ...
  stopifnot(
    "`identification_column_name` must be a character" = is.character(
      identification_column_name
    )
  )
  ...
  tryCatch(
    {
      ...
      if (identification_column_name != "scientific_name") {
        df <- df |>
          dplyr::rename(
            scientific_name := dplyr::all_of(identification_column_name)
          )
      }
      ...
      taxonomic_lookup_df <- marinegeo.utils::db_get_taxonomy_by_scientific_name_deprecated(
        scientific_names,
        taxonomic_levels
      )
    ...
    error = function(e) {
      cat(
        "Error in utl_join_taxonomy_by_scientific_name_deprecated:",
        e$message,
        "\n"
      )
      return(NULL)
...
#' @export
#' @examples
#' db_get_taxonomy_by_scientific_name_deprecated(c("Gastropoda", "Dardanus venosus"))
#'
#' db_get_taxonomy_by_scientific_name_deprecated(
#'   "Dardanus venosus",
#'   taxonomic_levels = "phylum"
#' )
#'
#' # A misspelled level is reported rather than silently dropped
#' db_get_taxonomy_by_scientific_name_deprecated(
#'   "Dardanus venosus",
#'   taxonomic_levels = "pilum"
#' )

db_get_taxonomy_by_scientific_name_deprecated <- function(
  scientific_names,
  taxonomic_levels = NULL
) {
  # Test inputs
  stopifnot(
    "`scientific_names` must be a character vector" = is.character(
      scientific_names
    )
  )
  ...
  tryCatch(
    {
      taxonomy_subset <- marinegeo_metadata$legacy_taxonomic_ids |>
        ...
        dplyr::left_join(
          marinegeo_metadata$legacy_taxonomic_classifications,
          by = "taxonomic_id"
        )

      no_matches <- scientific_names[
        !scientific_names %in% taxonomy_subset$scientific_name
      ]
      ...
      if (nrow(taxonomy_subset) == 0) {
        ...
        warning(
          length(no_matches),
          " scientific names could not be matched in taxonomic database"
        )
      }
      ...
      if (!is.null(taxonomic_levels)) {
        ...
        missing_columns <- dplyr::setdiff(
          taxonomic_levels,
          colnames(taxonomy_subset)
        )

        if (length(missing_columns) > 0) {
          allowed_columns <- tibble::tibble(
            column_names = colnames(taxonomy_subset)
          ) |>
            dplyr::filter(
              !column_names %in% c("taxonomic_id", "scientific_name", "level")
            ) |>
            dplyr::distinct() |>
            dplyr::pull(column_names)

          warning(paste(
            "The following taxonomic level(s) is not defined in the database table:",
            paste(missing_columns, collapse = ", ")
          ))

          message(
            "The following taxonomic levels are accepted: ",
            paste(allowed_columns, collapse = ", ")
          )
        }
    ...
    error = function(e) {
      cat(
        "Error in db_get_taxonomy_by_scientific_name_deprecated():",
        e$message,
        "\n"
      )
      return(NULL)
...
utl_rls_fill_missing_metadata_deprecated <- function(df) {
  ...
  fill_options <- marinegeo.utils::db_arrow_marinegeo(
    "reef-life-survey-data-marinegeo-v1"
  ) %>%
