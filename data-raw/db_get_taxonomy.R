...
#' Get taxonomic classifications by  taxonomic ID
#'
#' @param scientific_names Vector of taxonomic IDs to get classifications for
#' @param taxonomic_levels Vector of classification levels. By default, returns all available levels
#'
#' @returns dataframe of taxonomic classifications and taxonomic ID (usually Aphia ID)
#' @export
...
db_get_taxonomy_by_taxonomic_id <- function(taxonomic_ids,
                                            taxonomic_levels = NULL){

  # Test inputs
  stopifnot("`taxonomic_id` must be a numeric vector" = is.numeric(taxonomic_ids))

  tryCatch({

    taxonomy_subset <- marinegeo_resources$taxonomic_classifications |>
      dplyr::filter(taxonomic_id %in% taxonomic_ids)

    no_matches <- taxonomic_ids[!taxonomic_ids %in% taxonomy_subset$taxonomic_id]
    ...
    if(nrow(taxonomy_subset) == 0){
      ...
      message(
        length(no_matches), " IDs could not be matched in taxonomic database"
      )
    }
    ...
    if(!is.null(taxonomic_levels)){
      ...
      taxonomy_level_subset <- taxonomy_subset |>
        dplyr::select(
          dplyr::any_of(c("taxonomic_id", taxonomic_levels))
        )
    ...
    cat("Error in db_get_taxonomy_by_taxonomic_id():", e$message, "\n")
    return(NULL)
