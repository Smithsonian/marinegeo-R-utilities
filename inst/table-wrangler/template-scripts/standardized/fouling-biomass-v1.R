...
df_out <- df %>%
  # mutate(biomass_method = NA) %>% # panel preweighed or panel scrapped
...
# Make sure calculated field is accurate, if relevant method used
all(
  df_out$community_biomass_g == (df_out$panel_wet_weight_g - df_out$panel_weight_predeployment_g)
)
