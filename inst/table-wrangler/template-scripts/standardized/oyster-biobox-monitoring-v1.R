# Process Oyster Biobox Monitoring Data 
...
table_out <- 'oyster-biobox-monitoring-v1'
...
df_out <- df%>%
  mutate(biobox_deployment_date =ymd(paste(deployment_year, deployment_month, deployment_day, sep = "-")),
         biobox_recovery_date = ymd(paste(recovery_year, recovery_month, recovery_day, sep = "_")))# %>%
#left_join(
#  marinegeo.utils::utl_mg_get_registry("site_codes") %>%
#    select(partner_code, site_code, site_name), by = "site_name"
#) %>%
#mutate(sample_event_id = paste(partner_code, site_code, year(biobox_deployment_date), sep = "_")) %>%
