# Mock taxonomic classifications table (wide form, as produced by
# .get_taxonomic_classifications() and stored at build time).
mock_classifications <- tibble::tribble(
  ~scientific_id , ~rank     , ~Kingdom   , ~Phylum        , ~Class           , ~Order        , ~Family       , ~Genus       , ~Species                 ,
  "APHIA:495077" , "Species" , "Plantae"  , "Tracheophyta" , "Liliopsida"     , "Alismatales" , "Zosteraceae" , "Zostera"    , "Zostera marina"         ,
  "APHIA:374534" , "Species" , "Plantae"  , "Tracheophyta" , "Liliopsida"     , "Alismatales" , "Zosteraceae" , "Zostera"    , "Zostera noltei"         ,
  "APHIA:125476" , "Species" , "Animalia" , "Chordata"     , "Actinopterygii" , "Perciformes" , "Labridae"    , "Thalassoma" , "Thalassoma bifasciatum"
)
...
test_that("result is a data frame", {
  local_mocked_bindings(
    marinegeo_metadata = list(taxonomic_classifications = mock_classifications),
    .package = "marinegeo.utils"
  )

  result <- utl_mg_get_taxonomic_classifications("APHIA:495077")
...
test_that("multiple IDs return one row each", {
  ...
  result <- utl_mg_get_taxonomic_classifications(
    c("APHIA:495077", "APHIA:374534", "APHIA:125476")
  )
...
test_that("different kingdoms return correct values in the same columns", {
  ...
  result <- utl_mg_get_taxonomic_classifications(
    c("APHIA:495077", "APHIA:125476")
  )

  plant_row <- result[result$scientific_id == "APHIA:495077", ]
...
test_that("NAs mixed with valid IDs: NAs removed with message, valid IDs processed", {
  ...
  expect_message(
    result <- utl_mg_get_taxonomic_classifications(
      c(NA_character_, "APHIA:495077", "APHIA:374534")
    ),
    "1 NA value\\(s\\) removed"
  )

  expect_equal(nrow(result), 2)
  expect_setequal(result$scientific_id, c("APHIA:495077", "APHIA:374534"))
})
